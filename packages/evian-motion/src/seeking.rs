    ...
    > Seeking<L, A>
{
    ...
    pub fn follow<
        ...
        I: Iterator<Item = Vec2<f64>> + Unpin,
    >(
        &mut self,
        drivetrain: &'a mut Drivetrain<Differential, T>,
        lookahead: f64,
        points: impl IntoIterator<Item = impl Into<Point2<f64>>, IntoIter = I>,
    ) -> FollowFuture<'a, L, A, T, I> {
        FollowFuture {
            sleep: sleep(Duration::from_millis(5)),
            drivetrain,
...
impl<
        ...
        T: TracksPosition + TracksHeading + TracksVelocity,
        I: Iterator<Item = Vec2<f64>> + Unpin,
    > FollowFuture<'_, L, AngularPid, T, I>
{
    pub const fn with_angular_gains(&mut self, kp: f64, ki: f64, kd: f64) -> &mut Self {
        self.angular_controller.set_gains(kp, ki, kd);
        self
...
impl<
        ...
        T: TracksPosition + TracksHeading + TracksVelocity,
        I: Iterator<Item = Vec2<f64>> + Unpin,
    > Future for FollowFuture<'_, L, A, T, I>
{
    type Output = ();
